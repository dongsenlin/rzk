...
async function stills(list) {
  ...
  for (const t of list) {
    ...
    await png(buf, path.join(dir, `f${String(f).padStart(3, '0')}.png`));
...
async function sheet(step) {
  ...
  await Promise.all(Array.from({ length: workers }, async () => {
    const b = await launch(), p = await page(b);
    while (queue.length) { const f = queue.shift(); await png(await frame(p, f, samples || 1), path.join(dir, `s${String(f).padStart(3, '0')}.png`)); }
    await b.close();
  }));
  const cols = 6;
  execFileSync(FFMPEG, ['-y', '-loglevel', 'error', '-pattern_type', 'glob', '-i', path.join(dir, 's*.png'),
    '-vf', `scale=480:-1,tile=${cols}x${Math.ceil(frames.length / cols)}:padding=4`, '-frames:v', '1', path.join(BUILD, 'sheet.png')]);
