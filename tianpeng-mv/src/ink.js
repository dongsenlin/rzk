// ─────────────────────────────────────────────────────────────────────────────
//  Ink painting kit: xuan paper and ink-ground textures, brush strokes with a
//  width profile and dry-brush bristles, washes with pooled edges, splatter,
//  gold leaf, silk ribbons, and an SVG path parser so figures can be written
//  as path data and painted with these brushes.
// ─────────────────────────────────────────────────────────────────────────────
'use strict';
...
const TEX = {};
// ── textures, generated once per page ──
function makePaper(size = 1600, seed = 11) {
  const cv = mk(size, size), c = cv.getContext('2d'), img = c.createImageData(size, size), d = img.data;
  const base = [230, 221, 201];
  for (let y = 0; y < size; y++) for (let x = 0; x < size; x++) {
    const n1 = fbm2(x / 260, y / 260, 4, seed), n2 = fbm2(x / 38, y / 38, 3, seed + 5), n3 = vnoise(x / 3.1, y / 3.1, seed + 9);
    const k = (n1 - 0.5) * 22 + (n2 - 0.5) * 9 + (n3 - 0.5) * 5;
    const o = (y * size + x) * 4;
    d[o] = base[0] + k; d[o + 1] = base[1] + k * 0.97; d[o + 2] = base[2] + k * 0.9; d[o + 3] = 255;
  }
  c.putImageData(img, 0, 0);
  // fibres: short bent hairs, slightly lighter and darker than the sheet
  const r = rng(seed);
  for (let i = 0; i < 2600; i++) {
    const x = r() * size, y = r() * size, a = r() * TAU, l = 6 + r() * 26;
    c.strokeStyle = r() < 0.5 ? 'rgba(255,252,242,0.35)' : 'rgba(120,105,80,0.12)'; c.lineWidth = 0.6 + r() * 0.6;
...
function makeNightGround(size = 1600, seed = 23) {           // dark mottled ink ground with white flecks (ref 3)
  const cv = mk(size, size), c = cv.getContext('2d'), img = c.createImageData(size, size), d = img.data;
  for (let y = 0; y < size; y++) for (let x = 0; x < size; x++) {
    const n1 = fbm2(x / 300, y / 300, 5, seed), n2 = fbm2(x / 60, y / 60, 3, seed + 3), n3 = vnoise(x / 2.5, y / 2.5, seed + 7);
    let v = 34 + (n1 - 0.5) * 46 + (n2 - 0.5) * 18 + (n3 - 0.5) * 6;
    const o = (y * size + x) * 4;
    d[o] = v; d[o + 1] = v * 0.99; d[o + 2] = v * 1.04; d[o + 3] = 255;
  }
  c.putImageData(img, 0, 0);
  const r = rng(seed);
  for (let i = 0; i < 900; i++) {                            // white flecks and scratches
    const x = r() * size, y = r() * size;
    c.fillStyle = `rgba(235,232,225,${0.08 + r() * 0.25})`;
    if (r() < 0.8) { c.beginPath(); c.arc(x, y, 0.5 + r() * 1.6, 0, TAU); c.fill(); }
    else { c.strokeStyle = c.fillStyle; c.lineWidth = 0.5; c.beginPath(); c.moveTo(x, y); c.lineTo(x + (r() - 0.5) * 30, y + (r() - 0.5) * 8); c.stroke(); }
  }
  return cv;
}
function makeGreyNoise(size = 512, scale = 24, seed = 5, oct = 4) {
  const cv = mk(size, size), c = cv.getContext('2d'), img = c.createImageData(size, size), d = img.data;
  for (let y = 0; y < size; y++) for (let x = 0; x < size; x++) {
    const v = fbm2(x / scale, y / scale, oct, seed) * 255, o = (y * size + x) * 4;
    d[o] = d[o + 1] = d[o + 2] = v; d[o + 3] = 255;
  }
  c.putImageData(img, 0, 0);
  return cv;
}
// alpha-noise mask: white with alpha = noise, for mottling washes
function makeAlphaNoise(size = 512, scale = 30, seed = 8, lo = 0.35, hi = 1) {
  const cv = mk(size, size), c = cv.getContext('2d'), img = c.createImageData(size, size), d = img.data;
  for (let y = 0; y < size; y++) for (let x = 0; x < size; x++) {
    const n = fbm2(x / scale, y / scale, 4, seed), o = (y * size + x) * 4;
    d[o] = d[o + 1] = d[o + 2] = 255; d[o + 3] = 255 * lerp(lo, hi, clamp((n - 0.25) / 0.5));
  }
  c.putImageData(img, 0, 0);
  return cv;
}
...
function PREPARE_INK() {
  TEX.paper = makePaper(1600, 11);
  TEX.night = makeNightGround(1600, 23);
  TEX.mottle = makeAlphaNoise(512, 26, 8, 0.0, 1.0);
  TEX.grain = makeGreyNoise(512, 3, 5, 2);
  TEX.cloud = makeTileNoise(512, 4, 41, 0.0, 1.0, 5);
}
