...
silk.png     2048² RGB   warm painting silk (绢本): fine plain weave, slubs, gentle tone drift, a few foxing spots, tileable
weave.png    2048² L     the same weave alone, as a multiply layer over what is painted on the silk
...
def main():
    ...
    # ── silk ──
    S2 = 2048
    xx, yy = np.meshgrid(np.arange(S2), np.arange(S2))
    jx = (tnoise(S2, 6, 131) - 0.5) * 2.2; jy = (tnoise(S2, 6, 132) - 0.5) * 2.2
    warp = 0.5 + 0.5 * np.sin(2 * np.pi * xx / 3.2 + jy)          # vertical threads
    weft = 0.5 + 0.5 * np.sin(2 * np.pi * yy / 3.2 + jx)          # horizontal threads
    cross = (warp * (1 - weft) + weft * (1 - warp))               # over-under
    slub = np.clip((tnoise(S2, 1.5, 133) - 0.62) * 6, 0, 1) * np.clip((tnoise(S2, 30, 134) - 0.4) * 3, 0, 1)
    tone = fbm(S2, 380, 4, 135)
    base = np.array([233, 216, 181], float); dark = np.array([214, 192, 150], float)
    img = dark + (base - dark) * np.clip(0.35 + tone * 0.8, 0, 1)[..., None]
    img *= (0.955 + 0.045 * cross)[..., None]
    img += (slub * 7)[..., None]
    fox = smooth(0.8, 0.86, fbm(S2, 26, 3, 136)) * smooth(0.55, 0.7, fbm(S2, 220, 3, 137))
    img = img * (1 - 0.12 * fox[..., None]) + np.array([150, 110, 70]) * 0.12 * fox[..., None]
    Image.fromarray(np.clip(img, 0, 255).astype(np.uint8), 'RGB').save(os.path.join(OUT, 'silk.png'))
    wv = 240 + 15 * cross - slub * 4 + (tnoise(S2, 0.7, 138) - 0.5) * 8
    Image.fromarray(np.clip(wv, 0, 255).astype(np.uint8), 'L').save(os.path.join(OUT, 'weave.png'))
    print('textures →', OUT)
